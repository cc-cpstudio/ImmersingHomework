...
public class UafPdfService
{
    ...
    private byte[] GeneratePdfInternal(List<UafPayload> payloads, DateOnly date, Dictionary<string, PdfDeviceRgb> tagColors)
    {
        ...
        var grouped = payloads.GroupBy(p => p.Subject);

        var title = $"{date.Month}月{date.Day}日作业";
        var titleWidth = _fontBold!.GetWidth(title, TitleFontSize);
        var black = new PdfDeviceRgb(0, 0, 0);
        DrawText(document, title, _fontBold, TitleFontSize, (PageWidth - titleWidth) / 2, currentY, black);
        ...
        foreach (var subjectGroup in grouped)
        {
            ...
            DrawText(document, subjectText, _fontMedium!, SubjectFontSize, Margin, currentY, black);
            ...
            foreach (var item in subjectGroup)
            {
                ...
                var contentText = $"{itemIndex}. {item.Content}";
                DrawText(document, contentText, _fontRegular!, ContentFontSize, Margin, currentY, black);
                ...
                if (item.Tags.Count > 0)
                {
                    ...
                    currentY = DrawTagsOnCanvas(pdfDoc, page, item.Tags, _fontRegular!, currentY, tagColors);
                }
            }
        }

        currentY -= ContentSpacing * 2;
        var footerText = "Generated by ImmersingHomework";
        ...
        DrawText(document, footerText, _fontRegular, FooterFontSize, (PageWidth - footerWidth) / 2, currentY, footerGray);
    ...
    private static void DrawText(Document document, string text, PdfFont font, float fontSize, float x, float yFromTop, PdfColor color)
    {
        ...
        var p = new Paragraph(text)
            ...
            .SetFontColor(color)
            .SetFixedPosition(x, yPdf, PageWidth - Margin * 2);
        document.Add(p);
    }

    private static float DrawTagsOnCanvas(PdfDocument pdfDoc, PdfPage page, IReadOnlyList<string> tags, PdfFont font, float yFromTop, Dictionary<string, PdfDeviceRgb> tagColors)
    {
        ...
        foreach (var tagName in tags)
        {
            ...
            var yPdf = PageHeight - rowYFromTop;
        ...
        var rows = (int)Math.Ceiling((tagStartYFromTop - rowYFromTop) / (TagHeight + TagRowSpacing)) + 1;
